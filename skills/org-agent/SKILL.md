...
1. 用户说"开启常驻 / 加常驻提示" → 模型读取 `templates/always-on.md`(仓库内通用模板,2 行常驻块,内容与工具无关)
2. **按目标工具追加**(幂等:先 grep 标题 `# org 经验组织系统` 是否已存在——存在则跳过并提示"已启用";不存在则追加(文件尾空行 + 模板内容)):
   - **Claude Code**: `~/.claude/CLAUDE.md`
   - **DSH**: `~/.dsh/AGENTS.md`(DSH 的常驻指令文件,agent-instructions 插件每次会话自动加载;用户级固定为 $DSH_HOME/AGENTS.md)
   - 其他工具:按其常驻机制(如 Codex 的 `~/.codex/AGENTS.md`)同理
3. 用户说"关闭常驻 / 移除常驻提示" → 模型按标题定位并移除该块(连同前后多余空行)
4. 说明:这是**唯一可选**的配置写入;启用后新会话常驻感知(每会话多 ~3 行上下文),关闭后恢复零侵入
5. 验证:追加后 Read 目标文件确认;移除后确认标题消失
